...
try {
  for (const name of ['monday', 'validation', 'outcomes', 'roster', 'roster.test', 'owner.test']) {
  ...
  process.exitCode = spawnSync(process.execPath, ['--test', path.join(output, 'owner.test.js'), path.join(output, 'roster.test.js')], { stdio: 'inherit' }).status ?? 1;
} finally {
