...
export default function Desk({ demo = false }: { demo?: boolean }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setLeads(prev => prev.map(x => x.id === id && !result.warning ? { ...x, outreach: mondayOutcome(draft.outcome as (typeof CALL_OUTCOMES)[number]), interest: draft.interest || x.interest, nextFollowup: draft.followupDate || x.nextFollowup } : x));
