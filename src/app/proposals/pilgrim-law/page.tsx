...
const SERVICES: Service[] = [
  ...
  // ── One-time
  {
    ...
    bullets: [
      ...
      "Includes AI intake chatbot",
      "CRM integration & attorney profiles",
    ],
  },
  {
    id: "crm-setup",
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 10, textAlign: "center", padding: "0 24px", maxWidth: "760px" }}>
        ...
        <div className="p1" style={{
          ...
          Marketing Proposal · February 25, 2026
        </div>
        ...
        <p className="p4" style={{ fontSize: "clamp(15px,1.8vw,18px)", color: MUTED, lineHeight: 1.55, margin: "0 0 36px" }}>
          $150M+ in settlements. Nearly 20 years of practice. Your digital presence should be just as formidable.
        ...
        <div className="p4" style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
          {[
            { val: "$150M+", lbl: "Settlements Secured" },
            { val: "~20 Yrs", lbl: "In Practice" },
            { val: "4", lbl: "Practice Areas" },
          ].map(s => (
            <div key={s.val} style={{
...
<table width="600" cellpadding="0" cellspacing="0" align="center" style="max-width:600px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 40px rgba(0,0,0,0.08);">
  ...
  <tr><td style="background:linear-gradient(135deg,#1a1514 0%,#2d1f1a 100%);padding:40px 40px 36px;text-align:center;">
    ...
    <h1 style="font-size:28px;font-weight:800;color:#ffffff;margin:0 0 8px;letter-spacing:-0.03em;">Pilgrim Law Group</h1>
...
// ─── Main checkout page ────────────────────────────────────────────────────
// IDs of the services included in the recommended package
const PACKAGE_IDS = ["seo-regional", "digital-ads", "website-monthly", "social-growth"];

export default function PilgrimLawProposal() {
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [packageAccepted, setPackageAccepted] = useState(false);
  const [contactName, setContactName] = useState("Pilgrim Law Group");
  const [contactEmail, setContactEmail] = useState("");
  const [submitState, setSubmitState] = useState<"idle" | "loading" | "success" | "error">("idle");
  const totalRef = useRef<HTMLElement>(null);

  const toggleService = (id: string) => {
    setPackageAccepted(false); // manual change exits package pricing
    setSelectedIds(prev => {
  ...
  // When package is accepted, website is free — exclude its price from the total
  const monthlyTotal = selectedServices
    .filter(s => s.billing === "monthly")
    .reduce((sum, s) => sum + (packageAccepted && s.id === "website-monthly" ? 0 : s.price), 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      subject: `[Pilgrim Law Proposal] New Service Selection — ${new Date().toLocaleDateString()}`,
    ...
    try {
      const r1 = await fetch("https://api.web3forms.com/submit", {
        method: "POST",
        ...
        <div style={{ maxWidth: "1060px", margin: "0 auto" }}>
          ...
          <div style={{ marginBottom: "40px" }}>
            ...
            <p style={{ fontSize: "15px", color: "rgba(255,255,255,0.50)", lineHeight: 1.65, margin: 0, maxWidth: "560px" }}>
              Based on our conversations and the goals you outlined, we put together this package specifically for your firm.
            </p>
          </div>
          ...
          <div style={{
            ...
            {/* Line items */}
            {[
              { name: "SEO Regional", desc: "Multi-county keyword targeting, premium content & competitor monitoring", price: "$1,797" },
              { name: "Digital Ads Management", desc: "Google PPC campaign management & optimization (ad spend billed separately)", price: "$250" },
              { name: "Website", desc: "3–5 page custom site, mobile-first, SEO-optimized, hosted & maintained", price: "$497", free: true },
              { name: "Social Media Content — Growth", desc: "8 custom posts/month across Instagram, Facebook & TikTok with attorney approval", price: "$500" },
            ].map((item, i, arr) => (
            ...
            <div style={{
              ...
              <div>
                ...
                <p style={{
                  ...
                  $2,547<span style={{ fontSize: "18px", fontWeight: 400 }}>/mo</span>
                </p>
              </div>
              ...
              {selectedServices.map((s, i) => (
                <div key={s.id} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", flexShrink: 0 }}>
                    {packageAccepted && s.id === "website-monthly" ? (
